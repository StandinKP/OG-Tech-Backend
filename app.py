from flask import Flask , render_template , session , redirect , request , url_for , flash , Response , make_response , Markup
...
mail = Mail(app) 
bcrypt = Bcrypt(app) 
razorpay_client = razorpay.Client(auth=(os.getenv("RAZORPAY_PRIVATE"), os.getenv("RAZORPAY_SECRET"))) 
...
def signup():
    ...
    if request.method == "POST":
        ...
        html_content="<h1><a href =" + link  +  ">Click Here to verify your account</a></h1>")
        try:
                sg = SendGridAPIClient(os.getenv("SENDGRID_KEY"))
                
                response = sg.send(message)
                mongo.db.users.update_one({"email" : user['email']},{"$set": {"token": token }}) 
                flash("We've sent you a verification mail . Please check your mail to continue ." , "success")
                return(redirect(url_for("signup")))
        except Exception as e:
                print(e)
                flash("Due to unknown reasons , the verification email could not be sent . Please try signing up again . ", "warning")
...
def create_card():
    if request.method == "POST":
        if request.form.get("email") == "" or request.form.get("name") == "" or request.form.get("contact") == "":
             flash("Please enter sufficient information","Danger")
             return(redirect(url_for("create_card")))
        else:
            card = {"creater_email":session["email"] ,
...
def convert(card):
    ...
    pdf = pdfkit.from_string(render_template("card2.html" , card = c),False ,css=css, configuration=config)
    response = make_response(pdf)
    response.headers["Content-Type"] = "application/pdf"
    response.headers["Content-Disposition"] = (
        "attachment; filename=" + "trial" +".pdf"
    )
